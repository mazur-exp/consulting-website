...
try {
  ...
  // Русский добавлен там же: server/vite.ts отдавал ?lang=ru из lang-ru/, но
  // этого дерева никто не собирал — и боту на ?lang=ru молча уезжал английский
  // снапшот. Русские тексты у нас самые полные, отдавать вместо них перевод
  // перевода — потеря на ровном месте.
  const LANGS = ['en', 'id', 'th', 'ru'];
  let count = 0;
