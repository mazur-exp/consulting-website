...
export default function AboutPage() {
  ...
  const facts: Array<[string, string]> = [
    [t('Юридическое лицо', 'Legal entity', 'Badan hukum'), 'PT Delivery Booster Group'],
    [t('Работаем с', 'Operating since', 'Beroperasi sejak'), '2023'],
    [t('На сопровождении сейчас', 'Under management today', 'Dalam pengelolaan saat ini'), '110+'],
    [t('Ресторанов прошло через нас с 2023', 'Restaurants served since 2023', 'Restoran yang kami tangani sejak 2023'), '200+'],
    [t('Основные рынки', 'Core markets', 'Pasar utama'), t('Бали (Индонезия), Пхукет (Таиланд)', 'Bali (Indonesia), Phuket (Thailand)', 'Bali (Indonesia), Phuket (Thailand)')],
    [t('Платформы', 'Platforms', 'Platform'), 'GrabFood, GoFood (GoJek), LINE MAN'],
    [t('Модель оплаты', 'Pricing model', 'Model pembayaran'), t('10% от выручки доставки, без предоплаты', '10% of delivery revenue, no upfront payment', '10% dari omzet delivery, tanpa pembayaran di muka')],
  ];
          ...
          <section className="py-16 sm:py-20">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              >
                ...
                {t('На главную', 'Home', 'Ke beranda')}
              </Link>

              <motion.div {...fadeIn}>
                <h1 className="text-3xl sm:text-5xl font-bold mb-6">
                  {t('О компании Delivery Booster', 'About Delivery Booster', 'Tentang Delivery Booster')}
                </h1>
              ...
              <motion.div {...fadeIn} className="glass-card p-6 sm:p-8 rounded-2xl mt-12">
                <h2 className="text-2xl font-bold mb-6">{t('Факты о компании', 'Company facts', 'Fakta tentang perusahaan')}</h2>
              ...
              <motion.div {...fadeIn} className="glass-card p-6 sm:p-8 rounded-2xl mt-12">
                <h2 className="text-2xl font-bold mb-4">
                  {t('Собственный софт: Delivery Booster App', 'Our software: Delivery Booster App', 'Software kami sendiri: Delivery Booster App')}
                </h2>
                ...
                <ul className="space-y-3 max-w-3xl">
                  {[
                    t('Live-статус: ресторан внезапно «закрылся» на платформе — вы узнаёте сразу, а не в конце дня', 'Live status: if the restaurant suddenly goes "closed" on a platform, you know immediately — not at the end of the day', 'Status live: kalau restoran tiba-tiba berstatus «tutup» di platform, Anda tahu saat itu juga — bukan di akhir hari'),
                    t('Защита рейтинга: мониторинг новых отзывов, выделение несправедливых, помощь с апелляциями на удаление', 'Rating protection: monitoring new reviews, flagging unfair ones, help with removal appeals', 'Perlindungan rating: pemantauan ulasan baru, penandaan ulasan yang tidak adil, bantuan mengajukan banding penghapusan'),
                    t('Контроль меню: позиции, случайно выключенные или недоступные на платформе', 'Menu control: items accidentally switched off or unavailable on the platform', 'Kontrol menu: item yang tidak sengaja dinonaktifkan atau tidak tersedia di platform'),
                    t('Ежедневные отчёты в Telegram и AI-ответы на отзывы клиентов', 'Daily Telegram reports and AI-generated replies to customer reviews', 'Laporan harian di Telegram dan balasan ulasan pelanggan yang dibuat AI'),
                  ].map((x, i) => (
                ...
                >
                  {t('Попробовать бесплатно — app.booster.delivery', 'Try it free — app.booster.delivery', 'Coba gratis — app.booster.delivery')}
                  <ExternalLink className="w-4 h-4" />
                </a>
                <p className="text-sm text-brand-muted mt-3">
                  {t('Первый месяц — все функции бесплатно; подключение по ссылке на ваш ресторан в Grab или GoFood.', 'First month free with all features; connect by pasting your restaurant link from Grab or GoFood.', 'Bulan pertama gratis dengan semua fitur; hubungkan dengan menempelkan tautan restoran Anda dari Grab atau GoFood.')}
                </p>
              </motion.div>
              ...
              <motion.div {...fadeIn} className="glass-card p-6 sm:p-8 rounded-2xl mt-8">
                <p className="text-sm uppercase tracking-wide text-brand-muted mb-5">
                  {t('Отзыв пользователя Delivery Booster App', 'What a Delivery Booster App user says', 'Ulasan pengguna Delivery Booster App')}
                </p>
                ...
                <p className="text-brand-muted mt-4 font-medium">
                  {t('— Евгений П., владелец ресторана', '— Evgeniy P., restaurant owner', '— Evgeniy P., pemilik restoran')}
                </p>
              </motion.div>
              ...
              <motion.div {...fadeIn} className="mt-12">
                <h2 className="text-2xl font-bold mb-4">
                  {t('Не путать с другими «бустерами»', 'Not to be confused with', 'Jangan disamakan dengan «booster» lain')}
                </h2>
                <p className="text-brand-muted max-w-3xl">
                  {t(
