    ...
    >
      <Block card>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
          <Stat value="96" label={t('ресторанов в выборке', 'restaurants in the sample', 'restoran dalam sampel')} />
          <Stat value="270 568" label={t('заказов за период', 'orders in the period', 'pesanan dalam periode')} />
          <Stat value="10.4x" label={t('медианный ROAS, Бали', 'median ROAS, Bali', 'median ROAS, Bali')} />
          <Stat value="25%" label={t('выручки теряется на простоях и стоп-листе', 'of revenue lost to downtime and stock-outs', 'omzet hilang karena offline dan stok habis')} />
        </div>
      </Block>

      <Block card title={t('1. Нормы рынка: Бали', '1. Market norms: Bali', '1. Norma pasar: Bali')}>
        ...
        <Table
          head={[t('Показатель', 'Metric', 'Metrik'), t('Медиана', 'Median', 'Median'), t('Половина флота (p25–p75)', 'Middle half (p25–p75)', 'Separuh tengah (p25–p75)')]}
          rows={[
            [t('Средний чек', 'Average check', 'Rata-rata nilai pesanan'), 'Rp 250k', 'Rp 194k – 296k'],
            [t('ROAS рекламы', 'Ads ROAS', 'ROAS iklan'), '10.4x', '7.9x – 12.7x'],
            [t('Доля рекламы в выручке', 'Ad spend as share of revenue', 'Porsi belanja iklan terhadap omzet'), '5.6%', '3.9% – 7.3%'],
            [t('Отмены', 'Cancellations', 'Pembatalan'), '0.35%', '0.10% – 1.10%'],
            [t('Заказов на 1 негативный отзыв', 'Orders per bad review', 'Pesanan per ulasan negatif'), '138', t('Grab 180 · GoFood 139', 'Grab 180 · GoFood 139', 'Grab 180 · GoFood 139')],
          ]}
        />
      ...
      <Block card title={t('2. Реклама: где начинается убывающая отдача',
        ...
        <Table
          head={[t('Доля рекламы в выручке', 'Ad share of revenue', 'Porsi iklan terhadap omzet'), t('Ресторанов', 'Restaurants', 'Restoran'), t('Медианный ROAS', 'Median ROAS', 'Median ROAS')]}
          rows={[
            ['0–2%', '7', '12.1x'],
            ['2–4%', '16', '12.8x'],
            ['4–6%', '26', '11.8x'],
            ['6–8%', '19', '9.6x'],
            [t('больше 8%', 'over 8%', 'di atas 8%'), '16', '6.9x'],
          ]}
        />
      ...
      <Block card title={t('3. Куда на самом деле утекает выручка',
        ...
        <Table
          head={[t('Источник потерь', 'Source of loss', 'Sumber kerugian'), t('Доля потерь', 'Share of losses', 'Porsi kerugian')]}
          rows={[
            [t('Позиции в стоп-листе', 'Items out of stock', 'Item dengan stok habis'), '95%'],
            [t('Простои (ресторан офлайн)', 'Downtime (restaurant offline)', 'Restoran offline'), '3%'],
            [t('Отмены заказов', 'Cancelled orders', 'Pesanan yang dibatalkan'), '2%'],
          ]}
        />
        <p className="text-brand-muted mt-6 max-w-3xl">
          {t(
      ...
      <Block card title={t('4. Отзывы: частота, распределение, снятие',
        ...
        <Table
          head={[t('Показатель', 'Metric', 'Metrik'), t('Значение', 'Value', 'Nilai')]}
          rows={[
            [t('Заказов на 1 негативный отзыв — медиана флота', 'Orders per bad review — fleet median', 'Pesanan per ulasan negatif — median armada'), '138'],
            [t('То же, Grab: среднее / лучший результат', 'Grab: average / best', 'Grab: rata-rata / terbaik'), '180 / 575'],
            [t('То же, GoFood: среднее / лучший результат', 'GoFood: average / best', 'GoFood: rata-rata / terbaik'), '139 / 236'],
            [t('Снимается по апелляции на Grab', 'Removed on appeal, Grab', 'Dihapus setelah banding, Grab'), '≈80%'],
          ]}
        />
        <p className="text-brand-muted mt-6 max-w-3xl">
          {t(
      ...
      <Block card title={t('5. Районы Бали различаются сильнее, чем кажется',
        ...
        <Table
          head={[t('Район', 'Area', 'Area'), t('Заведений', 'Venues', 'Tempat'), t('Медианный чек', 'Median check', 'Median nilai pesanan'), t('Медианный ROAS', 'Median ROAS', 'Median ROAS')]}
          rows={[
      ...
      <Block card title={t('6. Бали против Пхукета', '6. Bali versus Phuket', '6. Bali versus Phuket')}>
        <Table
          head={[t('Показатель', 'Metric', 'Metrik'), t('Бали', 'Bali', 'Bali'), t('Пхукет', 'Phuket', 'Phuket')]}
          rows={[
            [t('Медианный чек', 'Median check', 'Median nilai pesanan'), '≈$15.2', '≈$22.5'],
            [t('Медианный ROAS', 'Median ROAS', 'Median ROAS'), '10.4x', '22.8x'],
            [t('Доля рекламы в выручке', 'Ad share of revenue', 'Porsi iklan terhadap omzet'), '5.5%', '2.9%'],
            [t('Потери на простоях и стоп-листе', 'Downtime and stock-out losses', 'Kerugian dari offline dan stok habis'), '25%', '6%'],
          ]}
        />
        <p className="text-brand-muted mt-6 max-w-3xl">
          {t(
      ...
      <Block title={t('Метод и оговорки', 'Method and caveats', 'Metode dan catatan')}>
        <ul className="text-brand-muted text-sm space-y-2 max-w-3xl">
          <li>{t('— Выборка: 84 ресторана на Бали и 12 на Пхукете, которые Delivery Booster ведёт на GrabFood и GoFood; учтены только заведения с подключённым кабинетом мерчанта.',
                 ...
                 '— Sampel: 84 restoran di Bali dan 12 di Phuket yang dikelola Delivery Booster di GrabFood dan GoFood; hanya tempat dengan dasbor merchant terhubung yang dihitung.')}</li>
          <li>{t('— Период: январь–август 2026. Выручка, заказы, чек, расходы на рекламу, ROAS и отмены — фактические данные кабинетов GrabMerchant и GoBiz.',
                 ...
                 '— Periode: Januari–Agustus 2026. Omzet, pesanan, rata-rata nilai pesanan, belanja iklan, ROAS dan pembatalan adalah angka aktual dari dasbor GrabMerchant dan GoBiz.')}</li>
          <li>{t('— Упущенная выручка — расчётная оценка по методике Delivery Booster: простои и стоп-лист считаются по собственной скорости заказов каждого заведения, отмены — как количество × средний чек.',
                 ...
                 '— Omzet yang hilang adalah estimasi dengan metode Delivery Booster: restoran offline dan stok habis dihitung dari kecepatan pesanan tiap tempat; pembatalan sebagai jumlah × rata-rata nilai pesanan.')}</li>
          <li>{t('— Отзывы: выборка за 30 дней (432 отзыва) для распределения оценок; частота негатива и доля снятых по апелляции — за весь период.',
                 ...
                 '— Ulasan: sampel 30 hari (432 ulasan) untuk sebaran nilai; frekuensi ulasan negatif dan porsi yang dihapus lewat banding mencakup seluruh periode.')}</li>
          <li>{t('— Данные обезличены: публикуются только агрегаты по флоту и районам, без названий заведений и без цифр отдельных клиентов.',
                 ...
                 '— Data dianonimkan: hanya agregat tingkat armada dan area yang diterbitkan, tanpa nama tempat dan tanpa angka klien perorangan.')}</li>
          <li>{t('— Это выборка ресторанов под управлением агентства, а не случайная выборка рынка. Она смещена в сторону заведений, за которыми следят: реальные рыночные потери на стоп-листе, скорее всего, выше наших 25%.',
                 ...
                 '— Ini sampel restoran yang dikelola agensi, bukan sampel pasar acak. Sampel ini condong ke tempat yang diawasi: kerugian stok habis di pasar sebenarnya kemungkinan besar lebih tinggi dari 25% kami.')}</li>
        </ul>
      </Block>
      ...
      <Block>
        <p className="text-brand-muted">
          {t('Как мы работаем с этими цифрами: ', 'How we work with these numbers: ', 'Cara kami bekerja dengan angka ini: ')}
          ...
          <Link href="/answers/managing-grabfood-yourself" className="text-brand-green hover:underline">
            {t('сколько времени занимает вести это самому', 'how much time doing it yourself takes', 'berapa waktu yang dibutuhkan untuk mengurusnya sendiri')}
          </Link>
        </p>
      </Block>
