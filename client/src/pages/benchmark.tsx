    ...
    >
      <Block card>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
          <Stat value="96" label={t('ресторанов в выборке', 'restaurants in the sample')} />
          <Stat value="270 568" label={t('заказов за период', 'orders in the period')} />
          <Stat value="10.4x" label={t('медианный ROAS, Бали', 'median ROAS, Bali')} />
          <Stat value="25%" label={t('выручки теряется на простоях и стоп-листе', 'of revenue lost to downtime and stock-outs')} />
        </div>
      </Block>

      <Block card title={t('1. Нормы рынка: Бали', '1. Market norms: Bali')}>
        ...
        <Table
          head={[t('Показатель', 'Metric'), t('Медиана', 'Median'), t('Половина флота (p25–p75)', 'Middle half (p25–p75)')]}
          rows={[
            [t('Средний чек', 'Average check'), 'Rp 250k', 'Rp 194k – 296k'],
            [t('ROAS рекламы', 'Ads ROAS'), '10.4x', '7.9x – 12.7x'],
            [t('Доля рекламы в выручке', 'Ad spend as share of revenue'), '5.6%', '3.9% – 7.3%'],
            [t('Отмены', 'Cancellations'), '0.35%', '0.10% – 1.10%'],
            [t('Заказов на 1 негативный отзыв', 'Orders per bad review'), '138', t('Grab 180 · GoFood 139', 'Grab 180 · GoFood 139')],
          ]}
        />
      ...
      <Block card title={t('2. Реклама: где начинается убывающая отдача',
        ...
        <Table
          head={[t('Доля рекламы в выручке', 'Ad share of revenue'), t('Ресторанов', 'Restaurants'), t('Медианный ROAS', 'Median ROAS')]}
          rows={[
            ['0–2%', '7', '12.1x'],
            ['2–4%', '16', '12.8x'],
            ['4–6%', '26', '11.8x'],
            ['6–8%', '19', '9.6x'],
            [t('больше 8%', 'over 8%'), '16', '6.9x'],
          ]}
        />
      ...
      <Block card title={t('3. Куда на самом деле утекает выручка',
        ...
        <Table
          head={[t('Источник потерь', 'Source of loss'), t('Доля потерь', 'Share of losses')]}
          rows={[
            [t('Позиции в стоп-листе', 'Items out of stock'), '95%'],
            [t('Простои (ресторан офлайн)', 'Downtime (restaurant offline)'), '3%'],
            [t('Отмены заказов', 'Cancelled orders'), '2%'],
          ]}
        />
        <p className="text-brand-muted mt-6 max-w-3xl">
          {t(
      ...
      <Block card title={t('4. Отзывы: частота, распределение, снятие',
        ...
        <Table
          ...
          rows={[
            [t('Заказов на 1 негативный отзыв — медиана флота', 'Orders per bad review — fleet median'), '138'],
            [t('То же, Grab: среднее / лучший результат', 'Grab: average / best'), '180 / 575'],
            [t('То же, GoFood: среднее / лучший результат', 'GoFood: average / best'), '139 / 236'],
            [t('Снимается по апелляции на Grab', 'Removed on appeal, Grab'), '≈80%'],
          ]}
        />
        <p className="text-brand-muted mt-6 max-w-3xl">
          {t(
      ...
      <Block card title={t('5. Районы Бали различаются сильнее, чем кажется',
        ...
        <Table
          head={[t('Район', 'Area'), t('Заведений', 'Venues'), t('Медианный чек', 'Median check'), t('Медианный ROAS', 'Median ROAS')]}
          rows={[
      ...
      <Block card title={t('6. Бали против Пхукета', '6. Bali versus Phuket')}>
        <Table
          head={[t('Показатель', 'Metric'), t('Бали', 'Bali'), t('Пхукет', 'Phuket')]}
          rows={[
            [t('Медианный чек', 'Median check'), '≈$15.2', '≈$22.5'],
            [t('Медианный ROAS', 'Median ROAS'), '10.4x', '22.8x'],
            [t('Доля рекламы в выручке', 'Ad share of revenue'), '5.5%', '2.9%'],
            [t('Потери на простоях и стоп-листе', 'Downtime and stock-out losses'), '25%', '6%'],
          ]}
        />
        <p className="text-brand-muted mt-6 max-w-3xl">
          {t(
      ...
      <Block title={t('Метод и оговорки', 'Method and caveats')}>
        <ul className="text-brand-muted text-sm space-y-2 max-w-3xl">
          <li>{t('— Выборка: 84 ресторана на Бали и 12 на Пхукете, которые Delivery Booster ведёт на GrabFood и GoFood; учтены только заведения с подключённым кабинетом мерчанта.',
                 '— Sample: 84 restaurants in Bali and 12 in Phuket managed by Delivery Booster on GrabFood and GoFood; only venues with a connected merchant dashboard are counted.')}</li>
          <li>{t('— Период: январь–август 2026. Выручка, заказы, чек, расходы на рекламу, ROAS и отмены — фактические данные кабинетов GrabMerchant и GoBiz.',
                 '— Period: January–August 2026. Revenue, orders, check, ad spend, ROAS and cancellations are actual GrabMerchant and GoBiz dashboard figures.')}</li>
          <li>{t('— Упущенная выручка — расчётная оценка по методике Delivery Booster: простои и стоп-лист считаются по собственной скорости заказов каждого заведения, отмены — как количество × средний чек.',
                 '— Lost revenue is an estimate by the Delivery Booster method: downtime and stock-outs are computed from each venue’s own order speed; cancellations as count × average check.')}</li>
          <li>{t('— Отзывы: выборка за 30 дней (432 отзыва) для распределения оценок; частота негатива и доля снятых по апелляции — за весь период.',
                 '— Reviews: a 30-day sample (432 reviews) for the score distribution; negative frequency and appeal-removal share cover the full period.')}</li>
          <li>{t('— Данные обезличены: публикуются только агрегаты по флоту и районам, без названий заведений и без цифр отдельных клиентов.',
                 '— Data is anonymised: only fleet- and area-level aggregates are published, with no venue names and no individual client figures.')}</li>
          <li>{t('— Это выборка ресторанов под управлением агентства, а не случайная выборка рынка. Она смещена в сторону заведений, за которыми следят: реальные рыночные потери на стоп-листе, скорее всего, выше наших 25%.',
                 '— This is a sample of agency-managed restaurants, not a random market sample. It is biased toward venues that are being watched: real market stock-out losses are most likely higher than our 25%.')}</li>
        </ul>
      </Block>
      ...
      <Block>
        <p className="text-brand-muted">
          {t('Как мы работаем с этими цифрами: ', 'How we work with these numbers: ')}
          ...
          <Link href="/answers/managing-grabfood-yourself" className="text-brand-green hover:underline">
            {t('сколько времени занимает вести это самому', 'how much time doing it yourself takes')}
          </Link>
        </p>
      </Block>
