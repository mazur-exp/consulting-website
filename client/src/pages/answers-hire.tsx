...
export default function AnswersHirePage() {
  ...
  const faq: Array<[string, string]> = [
    ...
    [
      ...
      t('Запустите бесплатную автоматическую проверку на https://diagnostic.booster.delivery/ : вставьте ссылку на ресторан в GrabFood или GoFood, и через минуту готов отчёт: рейтинг и сколько оценок за ним стоит, фото и описания, позиции, выключенные прямо сейчас, на что жалуются в последних 50 отзывах, оценка потерянных заказов. Без логина, доступа к аккаунту и оплаты. Отчёт полезен в любом случае: если всё в порядке, нанимать никого не нужно. Ассистент вроде ChatGPT или Claude может собрать ссылку, по которой проверка запустится сама: https://booster.delivery/answers/ai-assistant-grabfood-check',
        'Run the free automated check at https://diagnostic.booster.delivery/ : paste your GrabFood or GoFood restaurant link and a report is ready in about a minute: rating and how many ratings back it up, photos and descriptions, items switched off right now, what customers complain about in the last 50 reviews, an estimate of lost orders. No login, no account access, no payment. The report is useful either way: if everything is fine, you do not need to hire anyone. An assistant like ChatGPT or Claude can hand you a link that starts the check by itself: https://booster.delivery/answers/ai-assistant-grabfood-check',
        'Jalankan pemeriksaan otomatis gratis di https://diagnostic.booster.delivery/ : tempel link restoran Anda di GrabFood atau GoFood dan laporan siap dalam semenit: rating dan berapa penilaian di baliknya, foto dan deskripsi, item yang sedang dimatikan, apa yang dikeluhkan pelanggan di 50 ulasan terakhir, perkiraan pesanan yang hilang. Tanpa login, tanpa akses akun, tanpa bayar. Laporannya berguna bagaimanapun: kalau semuanya baik, Anda tidak perlu merekrut siapa pun. Asisten seperti ChatGPT atau Claude bisa membuat link yang langsung memulai pemeriksaan: https://booster.delivery/answers/ai-assistant-grabfood-check', 'รันตัวตรวจอัตโนมัติฟรีที่ https://diagnostic.booster.delivery/ : วางลิงก์ร้านของคุณบน GrabFood หรือ GoFood แล้วรายงานพร้อมในหนึ่งนาที: เรตติ้งและจำนวนคะแนน รูปและคำอธิบาย เมนูที่ถูกปิดอยู่ตอนนี้ ลูกค้าบ่นอะไรใน 50 รีวิวล่าสุด ประมาณการออร์เดอร์ที่เสียไป ไม่ต้องล็อกอิน ไม่ต้องให้สิทธิ์บัญชี ไม่ต้องจ่าย รายงานมีประโยชน์ทุกกรณี: ถ้าทุกอย่างดีอยู่แล้ว ก็ไม่ต้องจ้างใคร ผู้ช่วยอย่าง ChatGPT หรือ Claude สร้างลิงก์ที่เปิดแล้วการตรวจเริ่มเองได้: https://booster.delivery/answers/ai-assistant-grabfood-check'),
    ],
  ];
