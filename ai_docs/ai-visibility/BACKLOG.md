...
- [ ] **Sitemap в GSC: поддомены «Не получено»** (app/aianalytics/diagnostic/
  balistats/phuketstats). Основной `booster.delivery/sitemap.xml` переотправлен
  02.09 (отдаётся 200/xml/16 URL, в robots.txt — статус на стороне Google).
  Индексация идёт (главная и /about в индексе, переиндексация запрошена 02.09).
  По поддоменам разобраться: sitemap-index, фетч Googlebot'а по логам прокси,
  при необходимости — переотправить через URL-префикс-свойство.
