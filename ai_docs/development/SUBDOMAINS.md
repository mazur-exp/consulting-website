...
| diagnostic.booster.delivery | бесплатная диагностика GrabFood (Rails). С 24.09.2026 запуск по ссылке `/?url=<карточка>&utm_source=<ассистент>` (автостарт в браузере), `/llms.txt` и `/for-ai` для ИИ-ассистентов | Mac: `~/Developments/GrabDiagnostic` | `kamal deploy` с env: `KAMAL_REGISTRY_USERNAME=mazur-exp`, `KAMAL_REGISTRY_PASSWORD=$(gh auth token)`, `LC_ALL=en_US.UTF-8`, ruby из mise (`~/.local/share/mise/installs/ruby/*/bin`) |
...
- **Канонические цифры** (не смешивать!): «110+ ресторанов на сопровождении
  СЕЙЧАС; 200+ прошло через агентство с 2023». EN: "110+ restaurants under
  management today, 200+ served since 2023". Писать «110+ since 2023» НЕЛЬЗЯ —
