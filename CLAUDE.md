...
**Длинное тире (—) запрещено везде: публичные тексты, письма, ответы Алексу.**
Только короткий дефис (-). Правило Алекса 18.09.2026: длинное тире выдаёт текст ИИ.
Проверка перед публикацией: `grep -c "—" файл` должна давать 0. Картинки для постов
рендерить самому и прикреплять к сообщению (CONTENT_PLAYBOOK §4).
